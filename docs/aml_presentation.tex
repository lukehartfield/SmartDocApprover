...
\begin{tikzpicture}[
    ...
    \node[layer=primary!12] (l1) {\textbf{Layer 1: Classification Ensemble} --- ViT + ResNet18 (Accuracy: 97\%)};
    \node[layer=success!12, below=of l1] (l2) {\textbf{Layer 2: OCR} --- EasyOCR with auto-retry (75\% high-confidence regions)};
    \node[layer=primary!12, below=of l2] (l3) {\textbf{Layer 3: Field Extraction Ensemble} --- LayoutLM + Regex + Position + NER (F1: 81\%)};
    \node[layer=warning!15, below=of l3] (l4) {\textbf{Layer 4: Anomaly Detection Ensemble} --- IsoForest + XGBoost + LogReg + SVM (Accuracy: 90\%)};
...
\midrule
ViT (Single Model) & 91.0\% & 92.0\% & 95.0\% & 0.967 \\
\textbf{Ensemble (ViT + ResNet)} & \textbf{97.0\%} & \textbf{98.4\%} & \textbf{98.3\%} & \textbf{0.994} \\
\midrule
\textit{Improvement} & \textit{+6.6\%} & \textit{+6.4\%} & \textit{+3.3\%} & \textit{+0.027} \\
\bottomrule
\end{tabular}
\caption{Ensemble improves classification accuracy by 6.6\%}
\end{table}

% FIGURE PLACEHOLDER
% \begin{figure}[H]
%     \centering
%     \includegraphics[width=0.95\textwidth]{../assets/images/classifier_evaluation.png}
%     \caption{Classification evaluation: confusion matrices and ROC curves}
% \end{figure}
...
\subsection{Results}
\begin{itemize}[itemsep=2pt]
    \item \textbf{75\% of text regions} detected with high confidence ($>$85\%)
    \item \textbf{20\% medium confidence} (50-85\%)
    \item \textbf{5\% low confidence} ($<$50\%) --- these are flagged
\end{itemize}

% FIGURE PLACEHOLDER
% \begin{figure}[H]
%     \centering
%     \includegraphics[width=0.8\textwidth]{../assets/images/ocr_evaluation.png}
%     \caption{OCR confidence distribution}
% \end{figure}
...
% \begin{figure}[H]
%     \centering
%     \includegraphics[width=0.9\textwidth]{../assets/images/layoutlm_field_extraction.png}
%     \caption{Field extraction metrics and radar chart}
% \end{figure}
...
Logistic Regression & 20\% & Simple linear decision boundary \\
...
\bottomrule
\end{tabular}
\caption{Ensemble outperforms all individual detectors}
\end{table}

% FIGURE PLACEHOLDER
% \begin{figure}[H]
%     \centering
%     \includegraphics[width=0.95\textwidth]{../assets/images/anomaly_detection_evaluation.png}
%     \caption{Anomaly detection: ROC curves and detector comparison}
% \end{figure}
...
\begin{itemize}[itemsep=2pt]
    \item \textbf{APPROVE}: Receipt detected ($>$70\% conf), all fields found, no anomalies
    \item \textbf{REVIEW}: Low confidence, missing fields, or anomaly detected
    \item \textbf{REJECT}: Not a receipt ($>$50\% confidence for ``other'' class)
\end{itemize}
...
\midrule
Document Classification & 91\% & \textbf{97\%} & +6.6\% \\
Field Extraction (F1) & 72\% & \textbf{81\%} & +9.0\% \\
Anomaly Detection & 85\% & \textbf{90\%} & +5.0\% \\
\bottomrule
\end{tabular}
...
\begin{enumerate}[itemsep=3pt]
    \item \textbf{Ensembles work}: 5-9\% improvement across all components
...
The result is a system that achieves \textbf{97\% classification accuracy} and \textbf{90\% anomaly detection accuracy}, while improving automatically from user feedback.

\vspace{1em}
\noindent\textbf{Repository}: \url{https://github.com/RogueTex/StreamingDataforModelTraining}

%=============================================================================
% APPENDIX: FIGURES (Uncomment after adding images to assets/images/)
%=============================================================================
...
%     \caption{Document classifier: confusion matrices, ROC curves, and performance metrics}
% \end{figure}
% 
% \begin{figure}[H]
%     \centering
%     \includegraphics[width=\textwidth]{../assets/images/ocr_evaluation.png}
%     \caption{OCR confidence distribution and quality breakdown}
% \end{figure}
% 
% \begin{figure}[H]
%     \centering
%     \includegraphics[width=\textwidth]{../assets/images/layoutlm_field_extraction.png}
%     \caption{Field extraction: per-field metrics and F1-score radar chart}
% \end{figure}
% 
% \begin{figure}[H]
%     \centering
%     \includegraphics[width=\textwidth]{../assets/images/anomaly_detection_evaluation.png}
%     \caption{Anomaly detection: ROC curves for all detectors and ensemble}
% \end{figure}
